...
import {
  ...
  Facebook, Instagram, Youtube, ArrowRight, Clock, ChevronDown,
} from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: `Reliable ${BIZ.service.toLowerCase()} in ${BIZ.city}. Trained staff, fully insured, 5-star rated. Get a free custom quote today.` },
      { property: "og:title", content: `${BIZ.service} in ${BIZ.city} | ${BIZ.name}` },
      { property: "og:description", content: `Trusted commercial cleaning across ${BIZ.city} and surrounding areas. Free quote in 24 hours.` },
          ...
          <div className="flex gap-6 transition-transform duration-500"
            ...
            {REVIEWS.map((r, i) => (
              <div key={i} className="shrink-0 w-full sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-1rem)] bg-card rounded-2xl p-6 shadow-card border">
                ...
                <a href="#" className="mt-4 inline-block text-sm font-semibold text-brand hover:underline">View profile →</a>
              </div>
            ))}
          </div>
        </div>
    ...
    <section className="py-20 lg:py-24">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="mt-6 space-y-4 text-lg text-muted-foreground max-w-2xl mx-auto">
          <p>
            We don't publish a fixed price list — because no two buildings are the same. Your quote depends on square footage, frequency, and scope. What you get is a clear, itemized proposal with no hidden fees.
          </p>
